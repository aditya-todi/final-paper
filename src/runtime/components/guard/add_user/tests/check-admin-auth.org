...
class TestCheckAuth(TestHarness):
    ...
    def test_check_auth(self):
        ...
        a_user = User(name=Name(val="user user"),
                      email=Email(val="user@gnu.org"),
                      roles=[Role.user])

        a_key = "randomadd_userkey"
        session = Session(user=user, role=Role.user, key=a_key)

        self.component.em.add_user(user)
        self.component.em.add_user(a_user)
